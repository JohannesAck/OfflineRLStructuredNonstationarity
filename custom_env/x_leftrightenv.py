...
class XLeftRightEnv(HiPEnvironment):
    parameter_names = ['phi']
    jittable_render = True

    def __init__(self, scheduler: EnvScheduler):
        """
        Environment where the agent starts randomly in a 1D space, the goal is on the left or right and the reward is dense.

        Actions are to move in increments in the x direction.
        """
        super().__init__()
    ...
    def observation_space(self, params: EnvParams) -> spaces.Box:
        ...
        return spaces.Box(-high, high, shape=(1,), dtype=jnp.float32)
    ...
    def get_goal(self, state: EnvState, params: EnvParams) -> jax.Array:
        return jnp.array([2 * (params.phi % 2) - 1.0])
    ...
    def reset_env(self, key: chex.PRNGKey, params: EnvParams) -> Tuple[chex.Array, EnvState]:
        pos = jax.random.uniform(key,  shape=(1,), minval=-self.space_size, maxval=self.space_size,)
        state = EnvState(
            pos=pos,
            last_pos=pos,
            time=0,
    ...
    def _render(self, state: EnvState) -> jnp.ndarray:
        ...
        state = state.replace(pos=jnp.array([state.pos, jnp.zeros_like(state.pos)]), 
                              last_pos=jnp.array([state.last_pos, jnp.zeros_like(state.last_pos)]))
        ...
        goal = jnp.array([2 * (state.hip % 2) - 1.0, jnp.zeros_like(state.hip)])
